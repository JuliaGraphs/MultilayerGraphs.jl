...
mutable struct NodeAlignedEdgeColoredGraph{T,U} <: AbstractSynchronizedEdgeColoredGraph{T,U}
    ...
    fadjlist::Vector{Vector{HalfEdge{<:MultilayerVertex,<:Union{Nothing,U}}}} # the forward adjacency list of the NodeAlignedEdgeColoredGraph. It is a vector of vectors of `HalfEdge`s. Its i-th element are the `HalfEdge`s that originate from `v_V_associations[i]`.
...
Return a null NodeAlignedEdgeColoredGraph with with vertex type `T` weighttype `U`. Use this constructor and then add Layers via the `add_layer!`method.
"""
function NodeAlignedEdgeColoredGraph(T::Type{<:Number}, U::Type{<:Number})
    return NodeAlignedEdgeColoredGraph{T,U}(
        LayerDescriptor{T,U}[],
