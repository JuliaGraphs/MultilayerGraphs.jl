...
function get_graph_of_layers(
    ...
    adjacency_matrix = reshape(
        [
            i != j ? (1 / norm_factor) * sum(wgt[:, :, i, j]) : 0.0 for
            i in 1:num_layers for j in 1:num_layers
        ],
        (num_layers, num_layers),
    )
    
    layers_graph = [getproperty(mg, layer_descriptor.name).graph for layer_descriptor in mg.layers ]
    return DiGraphOfGraphs(
        layers_graph, adjacency_matrix
    )
end
