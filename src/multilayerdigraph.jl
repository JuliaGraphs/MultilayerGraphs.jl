...
"""
function MultilayerDiGraph(num_layers::Int64, n_nodes::Int64, min_edges::Int64, max_edges::Int64, graph_types::Vector{DataType} )
...
function MultilayerDiGraph(n_nodes::Int64, T::Type{ <: Number}, U::Type{ <: Number} )
    ...
    return MultilayerDiGraph(adjacency_tensor,OrderedDict{Tuple{Int64,Int64}, Layer{T,U}}(),OrderedDict{Tuple{Int64,Int64}, Interlayer{T,U}}())
end
...
"""
function MultilayerDiGraph(layers::Vector{ <: Layer{T,U}};  default_interlayer::String  = "multiplex") where {T,U}    
    return MultilayerDiGraph(layers, Interlayer{T,U}[]; default_interlayer = default_interlayer)
end
...
"""
function MultilayerDiGraph(layers::Vector{ <: Layer{T,U }}, specified_interlayers::Vector{ <: Interlayer{T,U}};  default_interlayer::String  = "multiplex") where {T, U}    
    # Check that all layers and specified interlayers are directed
    specified_subgraphs = vcat(layers,specified_interlayers)
    all( [istrait(IsDirected{typeof(subgraph.graph)}) for subgraph in specified_subgraphs])  || throw(ErrorException("Not all the underlying layers' and interlayers' graphs are directed."))
    ...
    for layer in layers
        add_layer!(multilayerdigraph, layer; new_default_interlayers_type = default_interlayer)
    end
...
"""
function add_layer!(mg::M,new_layer::L; new_default_interlayers_type = "multiplex") where { T, U, G <: AbstractGraph{T}, M <: MultilayerDiGraph{T, U}, L <: Layer{T,U,G}}
    #Check that the layer is directed
    istrait(IsDirected{typeof(new_layer.graph)}) || throw(ErrorException("The `new_layer`'s underlying graph $(new_layer.graph) is undirected, so it is not compatible with a `MultilayerDiGraph`."))
...
"""
function specify_interlayer!(mg::M, new_interlayer::In; symmetric_interlayer_name::String = "interlayer_$(new_interlayer.layer_2)_$(new_interlayer.layer_1)") where { T, U, G <: AbstractGraph{T}, M <: MultilayerDiGraph{T, U}, In <: Interlayer{T,U,G}}

    istrait(IsDirected{typeof(new_interlayer.graph)})  || throw(ErrorException("The `new_interlayer`'s underlying graph $(new_interlayer.graph) is undirected, so it is not compatible with a `MultilayerDiGraph`."))
...
function specify_interlayer!(mg::M, layer_1::Symbol, layer_2::Symbol, graph::G; new_interlayer_name::String = "interlayer_$(layer_1)_$(layer_2)", symmetric_interlayer_name::String = "interlayer_$(layer_2)_$(layer_1)", forbidden_vertices::Tuple{Vararg{MultilayerVertex{T}}}, forbidden_edges::Tuple{Vararg{NTuple{2, MultilayerVertex{T}}}} ) where { T, U, G<: AbstractGraph{T}, M <: MultilayerDiGraph{T, U}} # 
    ...
    istrait(isDirected{G}) || throw(ErrorException("The new interlayer's underlying graph $(graph) is undirected, so it is not compatible with a `MultilayerDiGraph`."))
...
Graphs.degree(mg::M, v::V) where {T, M <: MultilayerDiGraph{T, <: Real}, V <: MultilayerVertex{T}} = indegree(mg, v) + outdegree(mg, v)
...
"""
function Graphs.add_edge!(mg::M, src::V, dst::V, weight::U) where { T, U <: Real, M <: MultilayerDiGraph{T,U}, V <: MultilayerVertex{T}}
    ...
    if _src_layer == _dst_layer
        ...
        istrait(IsWeighted{typeof(interlayerContainingEdge.graph)}) &&  istrait(IsWeighted{typeof(interlayerSymmetricContainingEdge.graph)}) || throw(ErrorException("You are trying to add a weighted edge to an unweighted interlayer. If you are convinced that the graph type of the interlayer to which the two vertices $(src) and $(dst) belong is weighted, then please submit an issue so that we may give the `IsWeighted` trait to the graph type."))
...
"""
function Graphs.add_edge!(mg::M, src::V, dst::V) where { T, U <: Real, M <: MultilayerDiGraph{T,U}, V <: MultilayerVertex{T}}
...
"""
Graphs.add_edge!(mg::M, me::E) where { T, U <: Real, M <: MultilayerDiGraph{T,U}, E <: MultilayerEdge{MultilayerVertex{T},U}} = add_edge!(mg, src(me), dst(me), weight(me))
...
"""
Graphs.add_edge!(mg::M, me::E) where { T, U, M <: MultilayerDiGraph{T,U}, E <: MultilayerEdge{MultilayerVertex{T},Nothing}} = add_edge!(mg, src(me), dst(me))
...
"""
function Graphs.rem_edge!(mg::M, src::V, dst::V) where { T, U, M <: MultilayerDiGraph{T,U}, V <: MultilayerVertex{T}}
...
function get_projected_monoplex_graph(mg::M) where { M <: MultilayerDiGraph}
    projected_monoplex_adjacency_matrix = dropdims(sum(mg.adjacency_tensor, dims = (3,4)), dims =(3,4)) #::Matrix{eltype(mg.adjacency_tensor)}
    return SimpleWeightedDiGraph{M.parameters[1], M.parameters[2] }(projected_monoplex_adjacency_matrix)
end
...
function get_overlay_monoplex_graph(mg::M) where {M<: MultilayerDiGraph}
    ...
    return SimpleWeightedDiGraph{M.parameters[1], M.parameters[2]}(projected_overlay_adjacency_matrix)
end
...
"""
function get_graph_of_layers(mg::M; normalization::String = "total_edges") where {M <: MultilayerDiGraph}
    num_layers = length(mg.layers)
    norm_factor = 1
    if cmp(normalization,"total_edges") == 0
        norm_factor = ne(mg)
    end
    adjacency_matrix = reshape([i != j ? (1/norm_factor) * sum(mg.adjacency_tensor[:,:,i,j])/2  : 0.0 for i in 1:num_layers for j in 1:num_layers ], (num_layers,num_layers))
    return DiGraphOfGraphs(getproperty.(collect(values(mg.layers)), :graph), adjacency_matrix)
end
