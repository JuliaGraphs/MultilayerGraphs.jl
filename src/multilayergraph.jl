...
) where {T,U,G<:AbstractGraph{T},M<:MultilayerGraph{T,U},L<:Layer{T,U,G}}
    # Check that the layer is directed
    !istrait(IsDirected{typeof(new_layer.graph)}) || throw(
        ErrorException(
            "The `new_layer`'s underlying graph $(new_layer.graph) is directed, so it is not compatible with a `MultilayerGraph`.",
        ),
    )
...
) where {T,U,G<:AbstractGraph{T},M<:MultilayerGraph{T,U},In<:Interlayer{T,U,G}}
...
function Graphs.degree(
    mg::M, v::V
) where {T,M<:MultilayerGraph{T,<:Real},V<:MultilayerVertex{T}}
...
function Graphs.add_edge!(
    mg::M, me::E
) where {T,U<:Real,M<:MultilayerGraph{T,U},E<:MultilayerEdge{MultilayerVertex{T},U}}
...
function Graphs.add_edge!(
    mg::M, me::E
) where {T,U,M<:MultilayerGraph{T,U},E<:MultilayerEdge{MultilayerVertex{T},Nothing}}
