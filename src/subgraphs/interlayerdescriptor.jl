...
struct InterlayerDescriptor{T,U,G} <: AbstractInterlayerDescriptor{T,U,G}
    ...
    function InterlayerDescriptor(
        ...
        transfer_vertex_metadata::Bool,
        weighttype::Type{U},
    ) where {T,U,G<:AbstractGraph{T}} #  edge_metadata_function::Function, edge_weight_function::Function, 
        (layer_1 != layer_2) || throw(
            ErrorException(
                "`layer_1` must be different from `layer_2`. Found $layer_1 and $layer_2 .",
            ),
        )
        ...
        ) # edge_weight_function, edge_metadata_function,
    end
end
...
function InterlayerDescriptor(
    ...
    name::Symbol=Symbol("interlayer_$(layer_1)_$(layer_2)"),
) where {T,U,G<:AbstractGraph{T}}
    return InterlayerDescriptor(
        name,
        layer_1,
        layer_2,
        null_graph,
...
end # edge_weight_function::Function = (src,dst) -> one(U), edge_metadata_function::Function = (src,dst) -> NamedTuple(),
